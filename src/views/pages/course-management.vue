<template>
    <div>
            ...
            <el-tab-pane label="套餐管理" name="combo">
                <div class="container">
                    <div class="course-layout">
                        <!-- 添加套餐管理的树状结构 -->
                        <div class="tree-container">
                            <el-tree :data="treeData" :props="{ label: 'name' }" @node-click="handleComboNodeClick" default-expand-all highlight-current />
                        </div>

                        <div class="table-container">
                            <el-button type="primary" :icon="Refresh" @click="handleRefresh">刷新</el-button>
                            <el-button type="warning" :icon="CirclePlusFilled" @click="comboDialogVisible = true">新增套餐</el-button>
                            ...
                            <el-table :data="comboData" style="width: 100%; margin-top: 20px" v-loading="comboLoading">
                                ...
                                <el-table-column prop="schoolName" label="所属校区" width="150" align="center" show-overflow-tooltip />

                                <el-table-column prop="courseNames" label="包含课程" width="300" align="center" show-overflow-tooltip />
                                <el-table-column prop="price" label="价格（元）" width="100" align="center" show-overflow-tooltip />
                                <el-table-column prop="info" label="备注" width="150" align="center" show-overflow-tooltip />
                                <el-table-column label="操作" width="180" fixed="right" align="center">
                                    <template #default="scope">
                                        <el-button size="small" type="primary" @click="handleEditCombo(scope.row)">编辑</el-button>
                                        <el-button size="small" type="danger" @click="handleDeleteCombo(scope.row)">删除</el-button>
                                    </template>
                            ...
                            <div class="pagination" style="margin-top: 20px; text-align: right">
                                <el-pagination
                                    v-model:current-page="comboPage.index"
                                    v-model:page-size="comboPage.size"
                                    :total="comboPage.total"
                                    @current-change="changeComboPage"
        ...
        <!-- 课程编辑弹窗 -->
        <el-dialog :title="isEdit ? '编辑课程' : '新增课程'" v-model="editModelVisible" width="700px" destroy-on-close :close-on-click-modal="false" @close="closeDialog">
                    ...
                    <el-col :span="12">
                        <el-form-item label="课程类别" prop="category">
                            <el-select v-model="formData.category" placeholder="请选择课程类别" style="width: 100%" filterable>
                                <el-option v-for="item in conventions.courseCategories" :key="item.id" :label="item.name" :value="item.id" />
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
        ...
        <!-- 套餐编辑弹窗 -->
        <el-dialog :title="isEditCombo ? '编辑套餐' : '新增套餐'" v-model="comboDialogVisible" width="700px" destroy-on-close :close-on-click-modal="false" @close="closeComboDialog">
            <el-form ref="comboFormRef" :model="comboFormData" :rules="comboRules" label-width="100px">
                ...
                <el-form-item label="所属校区" prop="schoolId">
                    <el-select v-model="comboFormData.schoolId" placeholder="请选择校区" style="width: 100%" @change="handleComboSchoolChange" filterable>
                        <el-option v-for="item in schoolOptions" :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                </el-form-item>
                <el-form-item label="包含课程" prop="courseIds">
                    <el-select v-model="comboFormData.courseIds" multiple placeholder="请选择课程" style="width: 100%" filterable>
...
onMounted(async () => {
    try {
        await Promise.all([getSchools(), getCourses(), getCombos(), getCourseOptions()]);
    } catch (error) {
...
const handleDelete = (row) => {
    ElMessageBox.confirm("确认删除该课程吗？", "警告", {
...
const handleDeleteCombo = (row) => {
    ElMessageBox.confirm("确认删除该套餐吗？", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
    }).then(async () => {
