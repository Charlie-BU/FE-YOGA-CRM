    ...
    <div class="search-container">
        <el-form ref="searchRef" :model="query" :inline="true">
            <el-form-item :label="item.label" :prop="item.prop" v-for="item in options">
                <!-- 文本框、下拉框、日期框 -->
                <el-input v-if="item.type === 'input'" v-model="query[item.prop]" :disabled="item.disabled" :placeholder="item.placeholder" clearable></el-input>
            ...
            <el-form-item>
                ...
                <el-button type="primary" :icon="Refresh" @click="resetForm(searchRef)">刷新</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
