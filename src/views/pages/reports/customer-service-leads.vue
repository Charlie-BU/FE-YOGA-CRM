<template>
    <div>
        <div class="container">
                    ...
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="校区">
                                <el-select v-model="queryParams.schoolId" placeholder="请选择校区" clearable style="width: 100%">
            ...
            <!-- 原有的表格部分 -->
            <el-table ref="tableRef" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" @row-click="handleRowClick" v-loading="loading">
                ...
                <template v-for="item in columns" :key="item.prop">
                    ...
                    <el-table-column v-else-if="item.prop === 'amount' && item.label === '金额（元）'" :label="item.label" :align="item.align">
            ...
            <div class="pagination" style="margin-top: 20px; text-align: right">
                <el-pagination v-model:current-page="page.index" v-model:page-size="page.size" :total="page.total" @current-change="changePage" layout="total, prev, pager, next"> </el-pagination>
            </div>
        </div>
    </div>
</template>
...
const initData = async () => {
    ...
    try {
        if (queryParams.timeRange.length === 0) {
            ...
            const now = new Date();
            // 获取当月第一天（修复时区问题）
            const startDate = new Date(now.getFullYear(), now.getMonth(), 1).toLocaleDateString("zh-CN", { year: "numeric", month: "2-digit", day: "2-digit" }).replace(/\//g, "-");
            // 获取当月最后一天（修复时区问题）
            const endDate = new Date(now.getFullYear(), now.getMonth() + 1, 0).toLocaleDateString("zh-CN", { year: "numeric", month: "2-digit", day: "2-digit" }).replace(/\//g, "-");
